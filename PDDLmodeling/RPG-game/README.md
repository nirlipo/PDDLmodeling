...
Once you model problem 3, you can use this [animation profile](pddl_template/animation_profile_problem3.pddl) to Planimate your solution as shown above. Keep in mind that I had to add an additional predicate ```cell``` in my ```domain``` file. Sometimes, in order to planimate, you end up adding extra redundant predicates that are useful only for the purpose of generating the planimation.

This animation profile works only for grid layouts (problem3), as we automatically distribute the cells as a grid by the indexes of the ```cell``` objects. See line 21 in the [animation profile](pddl_template/animation_profile_problem3.pddl:21). 
